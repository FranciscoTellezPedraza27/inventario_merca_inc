...
import 'package:inventario_merca_inc/modules/dashboard/actions/delete_oxxoa.dart';
import 'package:inventario_merca_inc/modules/dashboard/actions/edit_oxxoa.dart';
...
class OxxoAdultosTable extends StatefulWidget {
  const OxxoAdultosTable({Key? key}) : super(key: key);

  @override
  OxxoAdultosTableState createState() => OxxoAdultosTableState();
}

class OxxoAdultosTableState extends State<OxxoAdultosTable> {
  String _searchQuery = "";
...
void initState() {
  ...
  void actualizarTimeStamp() async {
    ...
    var docs = await instance.collection('oxxoadultos').get();
    
    for (var doc in docs.docs) {
      if (!doc.data().containsKey('timestamp')) {
        await instance.collection('oxxoadultos').doc(doc.id).update({
    ...
    return Container(
      ...
      child: ScrollbarTheme(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('oxxoadultos').orderBy('timestamp').snapshots(),
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context,
                          builder: (context) => EditOxxoAScreen(document: filteredData[index])
                        );
                      },
                    ),
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context,
                          builder: (context) => DeleteOxxoAScreen(document: filteredData[index])
                        );
                      },
                    ),
                  ],
