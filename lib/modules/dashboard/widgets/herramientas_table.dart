...
import 'package:inventario_merca_inc/modules/dashboard/actions/delete_herramientas.dart';
import 'package:inventario_merca_inc/modules/dashboard/actions/edit_herramientas.dart';
import 'package:inventario_merca_inc/modules/dashboard/actions/view_products.dart';
import 'package:remixicon/remixicon.dart';
import 'package:cached_network_image/cached_network_image.dart';

class HerramientasTable extends StatefulWidget {
  const HerramientasTable({Key? key}) : super(key: key);

  @override
  HerramientasTableState createState() => HerramientasTableState();
}

class HerramientasTableState extends State<HerramientasTable> {
  String _searchQuery = "";
...
void initState() {
  ...
  void actualizarTimeStamp() async {
    ...
    var docs = await instance.collection('herramientas').get();
    
    for (var doc in docs.docs) {
      if (!doc.data().containsKey('timestamp')) {
        await instance.collection('herramientas').doc(doc.id).update({
    ...
    return Container(
      ...
      child: ScrollbarTheme(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('herramientas').orderBy('timestamp').snapshots(),
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context, 
                          builder: (context) => EditHerramientasScreen(document: filteredData[index])
                        );
                      },
                    ),
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context, 
                          builder: (context) => DeleteHerramientasScreen(document: filteredData[index])
                        );
                      },
                    ),
                  ],
