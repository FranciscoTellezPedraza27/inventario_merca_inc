...
import 'package:inventario_merca_inc/modules/dashboard/actions/delete_produccion.dart';
import 'package:inventario_merca_inc/modules/dashboard/actions/edit_produccion.dart';
import 'package:inventario_merca_inc/modules/dashboard/actions/view_products.dart';
import 'package:remixicon/remixicon.dart';
import 'package:cached_network_image/cached_network_image.dart';

class ProduccionTable extends StatefulWidget {
  const ProduccionTable({Key? key}) : super(key: key);

  @override
  ProduccionTableState createState() => ProduccionTableState();
}

class ProduccionTableState extends State<ProduccionTable> {
  String _searchQuery = "";
...
void initState() {
  ...
  void actualizarTimeStamp() async {
    ...
    var docs = await instance.collection('produccion').get();
    
    for (var doc in docs.docs) {
      if (!doc.data().containsKey('timestamp')) {
        await instance.collection('produccion').doc(doc.id).update({
    ...
    child: Container(
      ...
      child: ScrollbarTheme(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('produccion').orderBy('timestamp').snapshots(),
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context,
                          builder: (context) => EditProduccionScreen(document: filteredData[index])
                        );
                      },
                    ),
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context,
                          builder: (context) => DeleteProduccionScreen(document: filteredData[index])
                        );
                      },
                    ),
                  ],
