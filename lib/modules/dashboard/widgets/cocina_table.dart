...
class CocinaTable extends StatefulWidget {
  const CocinaTable({Key? key}) : super(key: key);

  @override
  CocinaTableState createState() => CocinaTableState();
}

class CocinaTableState extends State<CocinaTable> {
  String _searchQuery = "";
...
void initState() {
  ...
  void actualizarTimeStamp() async {
    ...
    var docs = await instance.collection('cocina').get();
    
    for (var doc in docs.docs) {
      if (!doc.data().containsKey('timestamp')) {
        await instance.collection('cocina').doc(doc.id).update({
    ...
    child: Container(
      ...
      child: ScrollbarTheme(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('cocina').orderBy('timestamp').snapshots(),
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context,
                          builder: (context) => EditCocinaScreen(document: filteredData[index])
                        );
                      },
                    ),
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context,
                          builder: (context) => DeleteCocinaScreen(document: filteredData[index])
                        );
                      },
                    ),
                  ],
