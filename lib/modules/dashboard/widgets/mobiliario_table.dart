...
import 'package:inventario_merca_inc/modules/dashboard/actions/delete_mobiliario.dart';
import 'package:inventario_merca_inc/modules/dashboard/actions/edit_mobiliario.dart';
import 'package:inventario_merca_inc/modules/dashboard/actions/view_products.dart';
import 'package:remixicon/remixicon.dart';
import 'package:cached_network_image/cached_network_image.dart';

class MobiliarioTable extends StatefulWidget {
  const MobiliarioTable({Key? key}) : super(key: key);

  @override
  MobiliarioTableState createState() => MobiliarioTableState();
}

class MobiliarioTableState extends State<MobiliarioTable> {
  String _searchQuery = "";
...
void initState() {
  ...
  void actualizarTimeStamp() async {
    ...
    var docs = await instance.collection('mobiliario').get();
    
    for (var doc in docs.docs) {
      if (!doc.data().containsKey('timestamp')) {
        await instance.collection('mobiliario').doc(doc.id).update({
    ...
    child: Container(
      ...
      child: ScrollbarTheme(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('mobiliario').orderBy('timestamp').snapshots(),
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context, 
                          builder: (context) => EditMobiliarioScreen(document: filteredData[index]));
                      },
                    ),
                    IconButton(
                      ...
                      onPressed: () {
                        showDialog(
                          context: context,
                          builder: (context) => DeleteMobiliarioScreen(document: filteredData[index])
                        );
                      },
                    ),
                  ],
