...
class AddHerramientasScreen extends StatefulWidget {
  const AddHerramientasScreen({super.key});

  @override
  _AddHerramientasScreenState createState() => _AddHerramientasScreenState();
}

class _AddHerramientasScreenState extends State<AddHerramientasScreen> {
  ...
  Future<String?> _uploadImage(String docId) async {
    ...
    try {
      final ref = FirebaseStorage.instance.ref(
        'herramientas/$docId/${DateTime.now().millisecondsSinceEpoch}'
      );
  ...
  Future<void> _addHerramientas() async {
    if (!_formKey.currentState!.validate()) return;

    try {
      final docRef = FirebaseFirestore.instance.collection('herramientas').doc();
      final imageUrl = await _uploadImage(docRef.id);

      final nuevaHerramienta = {
