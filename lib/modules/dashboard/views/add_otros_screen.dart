...
class AddOtrosScreen extends StatefulWidget {
  const AddOtrosScreen({super.key});

  @override
  _AddOtrosScreenState createState() => _AddOtrosScreenState();
}

class _AddOtrosScreenState extends State<AddOtrosScreen> {
  ...
  Future<String?> _uploadImage(String docId) async {
    ...
    try {
      final ref = FirebaseStorage.instance.ref(
        'otros/$docId/${DateTime.now().millisecondsSinceEpoch}'
      );
  ...
  Future<void> _addOtros() async {
    if (!_formKey.currentState!.validate()) return;

    try {
      final docRef = FirebaseFirestore.instance.collection('otros').doc();
