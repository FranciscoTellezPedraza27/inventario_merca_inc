...
class AddLimpiezaScreen extends StatefulWidget {
  const AddLimpiezaScreen({super.key});

  @override
  _AddLimpiezaScreenState createState() => _AddLimpiezaScreenState();
}

class _AddLimpiezaScreenState extends State<AddLimpiezaScreen> {
  ...
  Future<String?> _uploadImage(String docId) async {
    ...
    try {
      final ref = FirebaseStorage.instance.ref(
        'limpieza/$docId/${DateTime.now().millisecondsSinceEpoch}'
      );
  ...
  Future<void> _addLimpieza() async {
    if (!_formKey.currentState!.validate()) return;

    try {
      final docRef = FirebaseFirestore.instance.collection('limpieza').doc();
